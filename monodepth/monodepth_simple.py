...
def normalize_np_array(npa):
    return (255*(npa - np.min(npa))/np.ptp(npa)).astype(int)   
...
def test_simple_with_image_and_labels(params, input_image, labels):
    """Test function."""
    ...
    normalized_labels = normalize_np_array(labels)
    return mean_squared_error(normalized_prediction, normalized_labels)
